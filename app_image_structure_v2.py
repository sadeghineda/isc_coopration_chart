...
def build_graph(org_data, expanded_deputies, expanded_managers):
    ...
    for dep_name in org_data["deputies"].keys():
        if "توسعه" in dep_name and "کسب" in dep_name:
            left_arm = dep_name
        elif "برنامه" in dep_name or "برنامه‌ریزی" in dep_name:
            right_arm = dep_name
    ...
    excluded_arms = [arm for arm in [left_arm, right_arm] if arm is not None]
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        st.markdown("### 🔍 اطلاعات دیباگ")
        
        # نمایش نام معاونت‌ها
        with st.expander("لیست همه معاونت‌ها"):
            for dep_name in org_data["deputies"].keys():
                st.text(dep_name)
